...
- `set_cover(postId, mediaId, alt?)` — muqova (`save_rewrite`/`set_seo` bilan bir xil egalik/holat qoidalari; `alt` → postning `coverAlt`). SEO rasmi (`meta.image`) bo'sh yoki eski muqovaga teng bo'lsa, u ham yangi muqovaga tenglanadi — ikkala locale'da (posts `meta.image` hook'i, OBLOG-47; admin SEO tab'idagi preview shu maydonni ko'rsatadi); javobda `metaImageUpdated`.
