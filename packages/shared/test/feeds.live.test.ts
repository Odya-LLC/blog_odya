 ...
 *   (yoki: RUN_FEED_CHECKS=1 pnpm vitest run test/feeds.live.test.ts)
 *
...
async function loadRobots(source: SourceSeed, origin: string): Promise<Robots> {
  ...
  if (res.status !== 200) {
    console.warn(`[${source.slug}] robots.txt HTTP ${res.status} — tekshirib bo'lmadi (ehtiyotkor rejim: rss_only bo'lishi kerak)`)
    return null
  }
  ...
  for (const source of sources) {
    it(`${source.name}`, { timeout: 300_000 }, async (ctx) => {
      ...
      for (const feed of source.feeds) {
        ...
        if (!robotsByOrigin.has(origin)) robotsByOrigin.set(origin, await loadRobots(source, origin))
        const robots = robotsByOrigin.get(origin)
        if (robots) expect(robots.isAllowed(feed.url, UA_TOKEN), `robots: ${feed.url}`).not.toBe(false)
        ...
        if (res.status === 403 && res.headers.get('cf-mitigated') === 'challenge' && KNOWN_CLOUDFLARE_CHALLENGE.has(source.slug)) {
          console.warn(`[${source.slug}] SKIP: ${feed.url} — Cloudflare challenge (403). Qo'lda tekshiring (docs/sources.md §8 #2).`)
          ctx.skip()
        }
        ...
        report.push(`  ${res.status} items=${parsed.items.length} latest=${latest} active=${feed.isActive} ${feed.url}`)
      ...
      if (source.fetchMode === 'rss_plus_page') {
        ...
        if (robots) expect(robots.isAllowed(pageUrl, UA_TOKEN), `robots: ${pageUrl}`).not.toBe(false)
        const res = await politeFetch(pageUrl, source)
        expect(res.status, pageUrl).toBe(200)
        const { document } = parseHTML(await res.text())
        const text = document.querySelector(source.selectors!.content)?.textContent?.replace(/\s+/g, ' ').trim() ?? ''
        expect(text.length, `${source.selectors!.content} @ ${pageUrl}`).toBeGreaterThanOrEqual(MIN_PAGE_TEXT)
