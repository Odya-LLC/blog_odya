...
async function resolve(params: Props['params']) {
  ...
  if (page === 1) permanentRedirect(categoryPath('uz-Latn', category))
  return { category, page }
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { category, page } = await resolve(params)
  return categoryMetadata({ locale: 'uz-Latn', slug: category, page })
}

export default async function CategoryPaginatedPage({ params }: Props) {
  const { category, page } = await resolve(params)
  return <CategoryView locale="uz-Latn" slug={category} page={page} />
}
