...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  ALTER TABLE "tags" ADD COLUMN "cyrl_locked" jsonb;
  ALTER TABLE "tags" ADD COLUMN "cyrl_stale" boolean DEFAULT false;
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  // Tartib: avval FK/indeks/ustunlar, keyin jadvallar (CASCADE FK ni oldinroq olib tashlaydi).
  await db.execute(sql`
  ALTER TABLE "payload_locked_documents_rels" DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_glossary_fk";
  ALTER TABLE "payload_locked_documents_rels" DROP CONSTRAINT IF EXISTS "payload_locked_documents_rels_translit_exceptions_fk";
  DROP INDEX IF EXISTS "payload_locked_documents_rels_glossary_id_idx";
  DROP INDEX IF EXISTS "payload_locked_documents_rels_translit_exceptions_id_idx";
  ...
  ALTER TABLE "tags" DROP COLUMN "cyrl_locked";
  ALTER TABLE "tags" DROP COLUMN "cyrl_stale";
