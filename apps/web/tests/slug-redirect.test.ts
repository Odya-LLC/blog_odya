...
describe('planSlugRedirects', () => {
  it('default — faqat lotin (prefikssiz) yo‘l: sayt /kr ni o‘zi lokalizatsiya qiladi', () => {
    expect(planSlugRedirects('/ai/eski', '/ai/yangi')).toEqual([
      { from: '/ai/eski', to: '/ai/yangi' },
    ])
  })
  it('lotin va kirill (/kr) prefikslari uchun juftlar', () => {
    expect(planSlugRedirects('/ai/eski', '/ai/yangi', ['', '/kr'])).toEqual([
      { from: '/ai/eski', to: '/ai/yangi' },
...
describe('createSlugRedirectHook', () => {
  ...
  it('slug o‘zgarganda 301 redirect yaratadi (default — lotin yo‘li)', async () => {
  ...
  it('prefixes: lotin + kirill (/kr) juftlari', async () => {
    const both = createSlugRedirectHook<Doc>({ buildPath, prefixes: ['', '/kr'] })
    const { rows, req } = mockPayload()
    await runHook(both, req, { id: 1, slug: 'eski' }, { id: 1, slug: 'yangi' })
    expect(rows.map(({ from, to, type }) => ({ from, to: to.url, type }))).toEqual([
      { from: '/news/eski', to: '/news/yangi', type: '301' },
